...
export class LLMManager {
	...
	public async getCompletion(options: LLMCompletionOptions): Promise<LLMCompletionResponse> {
		...
		if (!provider) {
			logger.error(`No LLM provider registered under name "${primaryName}". Attempting fallback "gemini"...`);
			const fallback = this.providers.get('gemini');
			if (!fallback) {
				throw new Error(`No LLM providers registered for "${primaryName}" or fallback "gemini".`);
			}
			provider = fallback;
		}
