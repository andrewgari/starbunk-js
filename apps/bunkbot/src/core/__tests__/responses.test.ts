...
describe('Response functions', () => {
	...
	describe('sendBotResponse', () => {
		it('should send a response with the bot identity', async () => {
			...
			expect(logger.debug).toHaveBeenCalledWith(expect.stringMatching(/Sending response/));
		});
		...
		it('should not send empty responses', async () => {
			...
			expect(logger.warn).toHaveBeenCalledWith(expect.stringMatching(/Empty response generated/));
		});
