...
export class LLMQueryService {
	...
	public async queryGameContext(
		...
		logger.info(
			`[LLMQueryService] Starting query for campaign "${campaign.name}" by user ${userId}. Query: "${query}"`,
		);
		try {
			...
			logger.error(
				'[LLMQueryService] Error in queryGameContext:',
	...
	private buildSystemPrompt(campaign: Campaign, adventure: AdventureMetadata, isGM: boolean): string {
		return (
			`You are a helpful assistant for the "${campaign.name}" campaign, ` +
