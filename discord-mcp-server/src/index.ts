...
class DiscordMCPServer {
      ...
      switch (request.params.name) {
        case 'send_discord_message':
          return await this.sendMessage(
            request.params.arguments?.channelId as string,
        ...
        case 'get_discord_channels':
          return await this.getChannels(request.params.arguments?.serverId as string);
  ...
  private async getMessages(channelId: string, limit: number = 10) {
    try {
      const channel = await this.discordClient.channels.fetch(channelId);
  ...
  private async getServers() {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get servers: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async getChannels(serverId: string) {
    try {
