...
describe('QdrantMemoryService', () => {
	...
	beforeEach(() => {
		...
		(mockQdrantService.getInstance as jest.MockedFunction<typeof QdrantService.getInstance>).mockReturnValue(mockQdrantInstance);
		(mockEmbeddingService.getInstance as jest.MockedFunction<typeof EmbeddingService.getInstance>).mockReturnValue(mockEmbeddingInstance);
	...
	describe('Conversation Memory Management', () => {
		...
		it('should generate enhanced context combining personality and conversation', async () => {
			...
			const result = await memoryService.generateEnhancedContext(
				'Test message',
				'user123',
				'channel456'
			);
