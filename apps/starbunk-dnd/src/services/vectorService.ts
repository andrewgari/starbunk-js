...
export class VectorServiceError extends Error {
	...
	static fromError(
		message: string,
		error: unknown,
		additionalDetails: Partial<VectorServiceErrorDetails> = {},
	): VectorServiceError {
...
export class VectorService {
	...
	public async generateVectorsFromTexts(namespace: string, texts: readonly TextWithMetadata[]): Promise<void> {
		try {
	...
	public async findSimilarTexts(
		...
		try {
			...
			const similarities = vectors.map((vec: Float32Array) =>
				this.embeddingService.calculateCosineSimilarity(vec, queryVector),
			);
	...
	public async searchCampaignContent(
		...
		try {
			...
			const playerResults = await this.findSimilarTexts(`${campaignId}_player`, query, { limit: options.limit });
			...
			if (options.includeGMContent) {
				...
				gmResults = await this.findSimilarTexts(`${campaignId}_gm`, query, { limit: options.limit });
			}
			...
			logger.error(
				'[VectorService] Error searching campaign content:',
				error instanceof Error ? error : new Error(String(error)),
			);
			throw error;
		}
