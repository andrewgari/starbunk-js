...
export async function getBotIdentityFromDiscord({
	...
	try {
		...
		if (useRandomMember) {
			...
			logger.debug(`Getting Discord identity for user ${userId} (${fallbackName}) in guild ${guildId || 'default'}`);

			if (guildId && message) {
				// Get server-specific identity
				const serverIdentity = await getServerSpecificIdentity(discordService, userId, guildId, fallbackName, forceRefresh);
				if (!serverIdentity) {
