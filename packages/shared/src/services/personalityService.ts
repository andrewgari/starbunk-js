...
export class PersonalityService {
	...
	private constructor() {
		if (VectorEmbeddingService) {
			...
			logger.debug(
				'[PersonalityService] VectorEmbeddingService not available, embedding generation will be disabled',
			);
		}
	}
		...
		try {
				...
				try {
					...
					if (Array.isArray(data)) {
						...
						if (this.personalityEmbedding.length < 10) {
							logger.warn(
								`[PersonalityService] Loaded embedding has suspiciously few dimensions (${this.personalityEmbedding.length}). Expected ~384 for MiniLM-L6-v2.`,
							);
						}
						logger.info(
							`[PersonalityService] Successfully loaded personality embedding from ${jsonPath} with ${this.personalityEmbedding.length} dimensions`,
						);
					...
					if (filename.endsWith('.json')) {
						logger.error(
							`[PersonalityService] Failed to load JSON personality embedding: ${jsonError instanceof Error ? jsonError.message : String(jsonError)}`,
						);
					}
				}
			}
			...
			if (filename.endsWith('.npy')) {
				...
				try {
					...
					this.personalityEmbedding = new Float32Array(
						dataBuffer.buffer,
					...
					if (this.personalityEmbedding.length < 10) {
						throw new Error(
							`Invalid NPY file: embedding length (${this.personalityEmbedding.length}) too small`,
						);
					}

					logger.info(
						`[PersonalityService] Successfully loaded personality embedding from ${npyPath} with ${this.personalityEmbedding.length} dimensions`,
					);
					return this.personalityEmbedding;
				} catch (_npyError) {
	...
	public async generatePersonalityEmbedding(
		...
		try {
			...
			if (saveToFile) {
				...
				// Determine filename
				const filename = typeof saveToFile === 'string' ? saveToFile : 'personality.json';

				const filePath = path.join(dataDir, filename);

				// Save as JSON
				if (filename.endsWith('.json')) {
					await fs.writeFile(filePath, JSON.stringify(Array.from(this.personalityEmbedding || [])), 'utf-8');
				} else {
