...
export const ConfigSchema = z.object({
	discord: z.object({
		...
		clientId: z.string().regex(DISCORD_ID_REGEX, 'Invalid Discord client ID format').optional(),
		guildId: z.string().regex(DISCORD_ID_REGEX, 'Invalid Discord guild ID format').optional(),
	}),
	server: z.object({
		healthPort: z.coerce
			.number()
	...
	security: z.object({
		...
		maxMessageLength: z.coerce
			...
			.default(2000),
		allowedBotDirectories: z.array(z.string()).default(['reply-bots']),
...
export function validateAndParseConfig(): BunkBotConfig {
	...
	try {
		...
		if (error instanceof z.ZodError) {
			...
			throw new Error(
				`Invalid configuration - check environment variables: ${error.errors.length} validation errors found`,
			);
		}
...
function validateCustomRules(config: BunkBotConfig): void {
	...
	if (config.server.nodeEnv === NodeEnvironment.Production) {
	...
	if (config.security.enablePathTraversalProtection) {
		const suspiciousDirectories = config.security.allowedBotDirectories.filter(
			(dir) => dir.includes('..') || dir.includes('/') || dir.includes('\\'),
		);
	...
	if (config.features.maxBotInstances > estimatedMaxBots) {
		logger.warn(
			`⚠️ Max bot instances (${config.features.maxBotInstances}) may exceed available memory. Recommended max: ${estimatedMaxBots}`,
		);
	}
}
