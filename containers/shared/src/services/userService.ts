...
export class UserService {
	...
	public async initialize(): Promise<void> {
		...
		try {
			...
			logger.error('❌ Failed to initialize User Service:', error instanceof Error ? error : new Error(String(error)));
			throw error;
		}
	}
	...
	public async getUserId(username: string): Promise<string | null> {
		...
		try {
			...
			logger.error(`Failed to get user ID for username ${username}:`, error instanceof Error ? error : new Error(String(error)));
			return null;
		}
	}
	...
	public async getUsername(userId: string): Promise<string | null> {
		...
		try {
			...
			logger.error(`Failed to get username for user ID ${userId}:`, error instanceof Error ? error : new Error(String(error)));
			return null;
		}
	}
	...
	public async getUserConfig(userId: string) {
		...
		try {
			...
			logger.error(`Failed to get user config for ${userId}:`, error instanceof Error ? error : new Error(String(error)));
			return null;
		}
	}
	...
	public async getAllUserIds(): Promise<Record<string, string>> {
		...
		const knownUsernames = ['Cova', 'Venn', 'Bender', 'Sig', 'Guy', 'Guildus', 'Deaf', 'Feli', 'Goose', 'Chad', 'Ian'];
		...
		for (const username of knownUsernames) {
			try {
				...
				logger.warn(`Failed to get user ID for ${username}:`, error instanceof Error ? error : new Error(String(error)));
			}
		}
