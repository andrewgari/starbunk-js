...
if [[ ${#TO_BUILD[@]} -eq 0 ]]; then
  if echo "$CHANGED_LINES" | grep -E "^(\.github/workflows/|package.json|docker/docker-compose.*\.yml|docker/podman-compose.*\.yml)" >/dev/null 2>&1; then
    TO_BUILD=("${CONTAINERS[@]}")
  fi
fi
...
if [[ "${SKIP_DOCKER:-0}" != "1" ]] && command -v docker >/dev/null 2>&1; then
  ...
  for name in "${TO_BUILD[@]}"; do
      echo "→ docker compose -f docker/docker-compose.yml build ${name}"
      docker compose -f docker/docker-compose.yml build "${name}"
  done
else
