...
export class MessageHandler {
  ...
  private async processForProfile(
    ...
    if (responseContent && responseContent.trim().length > 0) {
      ...
      this.socialBatteryService.recordMessage(profile.id, message.channelId, batteryConfig);
  ...
  private buildLlmContext(profile: CovaProfile, message: Message): LlmContext {
    ...
    // Get user facts
    const userFacts = this.memoryService.getUserFacts(profile.id, message.author.id);
