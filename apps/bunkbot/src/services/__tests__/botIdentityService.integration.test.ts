...
const createMockGuildMember = (
	...
	user: {
		...
		displayAvatarURL: jest
			.fn()
			.mockReturnValue(avatarUrl || `https://cdn.discordapp.com/avatars/${userId}/avatar.png`),
	} as User,
	displayAvatarURL: jest
		.fn()
		.mockReturnValue(
...
describe.skip('BotIdentityService - Server-Specific Identity Resolution - DISABLED: Flaky service dependencies', () => {
	...
	describe('Cache Behavior with Server Context', () => {
		it('should cache identities per server', async () => {
			...
			mockDiscordService.getMemberAsync.mockResolvedValueOnce(member1).mockResolvedValueOnce(member2);
	...
	describe('Cache Management', () => {
		it('should clear user cache across all servers', async () => {
			...
			mockDiscordService.getMemberAsync.mockResolvedValue(createMockGuildMember(userId, 'Chad', 'Test Chad'));
