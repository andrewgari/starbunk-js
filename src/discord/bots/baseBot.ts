...
export abstract class BaseBot implements MessageHandler {
  ...
  protected abstract processMessage(message: Message): Promise<Result<void, Error>>;
  
  protected isOwnMessage(message: Message): boolean {
    return message.author.bot && message.author.username === this.config.name;
  }
  
  protected async sendReply(channel: TextChannel, content: string): Promise<Result<void, Error>> {
    ...
    try {
      ...
      return new Failure(error instanceof Error ? error : new Error('Failed to send message'));
    }
  }
} 