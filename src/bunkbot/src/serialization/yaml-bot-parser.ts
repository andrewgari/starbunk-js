  ...
  .discriminatedUnion('type', [
    ...
    z.object({
      ...
      as_member: z
        .string()
        .regex(/^\d{17,19}$/)
        .describe('The Discord User ID of the person to mimic.'),
    }),
    z.object({
      type: z
        .literal('random')
        .describe('Pick a random member of the server to "possess" for this message.'),
    }),
  ])
...
export const triggerSchema = z.object({
  ...
  responses: z
    ...
    .optional()
    .describe(
      'What the bot says when this trigger fires. If empty, it uses the bot-level responses.',
    ),
});
...
export const botSchema = z.object({
  ...
  triggers: z
    ...
    .describe('A list of independent rules for the bot to follow.'),
  ignore_bots: z
    .boolean()
    .default(true)
    .describe('If true, the bot will not respond to other bots.'),
  ignore_humans: z
    .boolean()
    .default(false)
    .describe('If true, the bot will only respond to other bots.'),
});
