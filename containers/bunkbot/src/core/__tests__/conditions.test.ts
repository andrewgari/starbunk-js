...
import { and, containsPhrase, containsWord, createChannelId, createDuration, createUserId, fromBot, fromUser, inChannel, matchesPattern, not, or, withChance, withinTimeframeOf, isE2ETestClient } from '../conditions';
...
describe('Condition functions', () => {
	...
	describe('isE2ETestClient', () => {
		...
		it('should return true when message is from same client ID (runtime detection)', () => {
			...
			const result = isE2ETestClient(message);
			expect(result).toBe(true);
		});

		it('should return false when message is from different client ID', () => {
		...
		it('should return true when message author username matches test client names', () => {
			const message = mockMessage({
				content: 'test',
		...
		it('should return true when message author display name matches test client names', () => {
			const message = mockMessage({
				content: 'test',
				author: mockUser({ 
		...
		it('should match test client names case-insensitively', () => {
			const message = mockMessage({
				content: 'test',
				author: mockUser({ id: '123456789012345678', username: 'testbot' })
			});
		...
		it('should return true when message author ID matches E2E_TEST_USER_ID env var', () => {
			const testUserId = '123456789012345678';
			process.env.E2E_TEST_USER_ID = testUserId;

			const message = mockMessage({
				content: 'test',
				author: mockUser({ id: testUserId })
			});
		...
		it('should return true when message author ID matches TEST_BOT_USER_ID env var', () => {
			const testUserId = '123456789012345678';
			process.env.TEST_BOT_USER_ID = testUserId;
		...
		it('should return false for non-test client messages', () => {
			const message = mockMessage({
				content: 'test',
				author: mockUser({ 
					id: '123456789012345678', 
		...
		it('should handle null/undefined message gracefully', () => {
			const result = isE2ETestClient(null as any);
			expect(result).toBe(false);
		});
		...
		it('should prioritize runtime detection over other methods', () => {
			const testClientId = '123456789012345678';
			const message = mockMessage({
				content: 'test',
				author: mockUser({ 
					id: testClientId, 
					username: 'NotATestBot' // This would normally not match
				}),
				client: {
					user: mockUser({ id: testClientId })
				}
			...
			const result = isE2ETestClient(message);
			expect(result).toBe(true); // Should be true due to runtime detection
		});

		it('should match all predefined test client names', () => {
			const testNames = ['TestBot', 'E2EBot', 'TestClient', 'E2E Test Bot'];
			
			testNames.forEach(testName => {
				const message = mockMessage({
					content: 'test',
					author: mockUser({ id: '123456789012345678', username: testName })
				});
