...
export class BotIdentityService {
		...
		try {
			...
			if (userId) {
				...
				if (identity) {
					...
					logger.warn(
						`Failed to get Discord identity for user '${username}' (${userId}), no fallback provided`,
					);
					return null;
				}
			} else {
	...
	async getBotIdentityByUserId(userId: string, fallbackName?: string): Promise<BotIdentity | null> {
		logger.warn(
			'getBotIdentityByUserId is deprecated. Use getBotIdentityByUserIdWithContext for server-specific identities.',
		);
