...
export class BotConfigurationService {
	...
	async loadConfiguration(): Promise<void> {
		...
		try {
			...
			logger.error(
				`[BotConfiguration] Failed to load configuration: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
			throw error;
		}
	}
	...
	private createDefaultConfiguration(): BotConfiguration {
		return {
			...
			corePersonality:
				'I am CovaBot, a helpful and friendly AI assistant. I aim to be supportive, encouraging, and provide useful information while maintaining a warm and approachable personality.',
			createdAt: new Date(),
			updatedAt: new Date(),
		};
	}
	...
	private async saveConfiguration(): Promise<void> {
		...
		try {
			...
			logger.error(
				`[BotConfiguration] Failed to save configuration: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
			throw error;
		}
	}
