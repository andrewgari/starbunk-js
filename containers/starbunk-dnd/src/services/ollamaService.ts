...
Format your response exactly like this example:
{
	...
	"isGMContent": false
		...
		try {
			...
			logger.error('Error parsing Ollama categorization response:', error instanceof Error ? error : new Error(String(error)));
			return {
				category: 'general',
				suggestedTags: [],
				isGMContent: false
...
Return ONLY the numbers of the most relevant pieces (max 3) as a JSON array like this: [1, 4, 7]`;
		...
		try {
			...
			logger.error('Error parsing Ollama context selection response:', error instanceof Error ? error : new Error(String(error)));
			return [1];
		}
	}
