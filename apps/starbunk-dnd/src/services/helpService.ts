...
export class HelpService {
		...
		this.helpContent = [
			{
				...
				tips: ['Choose a descriptive name for your campaign', 'Make sure to select the correct game system'],
			},
			{
				title: 'Note Taking',
	...
	public async initialize(): Promise<void> {
		try {
			...
			logger.error(
				'[HelpService] Error generating help content vectors:',
		...
		try {
			...
			return results
				.map((result) => {
					...
					const content = this.helpContent.find(
						(h) => h.command === (result.metadata as any).command || h.command === result.metadata.file,
					);
					return content;
				})
				.filter((content): content is HelpContent => content !== undefined && (isGM || !content.isGMOnly));
		} catch (error) {
			logger.error(
				'[HelpService] Error getting relevant help:',
				error instanceof Error ? error : new Error(String(error)),
			);
			return [];
		}
