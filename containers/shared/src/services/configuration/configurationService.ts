...
export class ConfigurationService {
	...
	public async initialize(): Promise<void> {
		...
		try {
			...
			logger.error('❌ Failed to initialize Configuration Service:', error instanceof Error ? error : new Error(String(error)));
	...
	public async getBotConfiguration(botName: string): Promise<BotConfig | null> {
		...
		try {
			...
			logger.error(`Failed to get bot configuration for ${botName}:`, error instanceof Error ? error : new Error(String(error)));
	...
	public async getAllBotConfigurations(): Promise<BotConfig[]> {
		...
		try {
			...
			logger.error('Failed to get all bot configurations:', error instanceof Error ? error : new Error(String(error)));
	...
	public async getUserConfiguration(userId: string): Promise<UserConfig | null> {
		...
		try {
			...
			logger.error(`Failed to get user configuration for ${userId}:`, error instanceof Error ? error : new Error(String(error)));
			throw error;
		}
	}
	...
	public async getUserByUsername(username: string): Promise<UserConfig | null> {
		...
		try {
			...
			logger.error(`Failed to get user configuration for username ${username}:`, error instanceof Error ? error : new Error(String(error)));
			throw error;
		}
	}
	...
	public async getServerConfiguration(serverId: string): Promise<ServerConfig | null> {
		...
		try {
			...
			logger.error(`Failed to get server configuration for ${serverId}:`, error instanceof Error ? error : new Error(String(error)));
			throw error;
		}
	}
	...
	private transformBotConfiguration(config: any): BotConfig {
		return {
			...
			triggers: config.patterns.map((pattern: any): BotTriggerConfig => ({
				name: pattern.name,
			...
			responses: config.responses.map((response: any): BotResponseConfig => ({
				name: response.name,
	...
	private getFallbackBotConfigurations(): BotConfig[] {
		return [
			this.getFallbackBotConfiguration('nice-bot'),
		].filter(Boolean) as BotConfig[];
	}
}
