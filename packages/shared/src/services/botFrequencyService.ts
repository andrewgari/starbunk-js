...
export class BotFrequencyService {
	...
	public async getBotFrequency(botName: string): Promise<number> {
		try {
			...
			logger.error(
				`[BotFrequencyService] Error getting frequency for ${botName}:`,
	...
	public async setBotFrequency(botName: string, frequency: number): Promise<void> {
		try {
			...
			logger.error(
				`[BotFrequencyService] Error setting frequency for ${botName}:`,
				error instanceof Error ? error : new Error(String(error)),
			);
			throw error;
		}
	...
	public async resetBotFrequency(botName: string): Promise<void> {
		try {
			...
			logger.error(
				`[BotFrequencyService] Error resetting frequency for ${botName}:`,
				error instanceof Error ? error : new Error(String(error)),
			);
			throw error;
		}
