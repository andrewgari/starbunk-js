...
describe.skip('BunkBot Container Health Check - DISABLED: Flaky in CI (requires Podman/Docker runtime)', () => {
	...
	beforeAll(async () => {
		...
		try {
			await execAsync(`podman build -f containers/bunkbot/Dockerfile -t ${CONTAINER_IMAGE} .`, {
	...
	it(
		'should start container and complete initialization successfully',
		async () => {
			...
			const { stdout: status } = await execAsync(
				`podman ps --filter name=${CONTAINER_NAME} --format "{{.Status}}"`,
			);
	...
	it(
		'should load reply bots without critical errors',
		async () => {
	...
	it(
		'should handle environment configuration correctly',
		async () => {
	...
	it(
		'should establish Discord connection without authentication errors',
		async () => {
	...
	it(
		'should not crash or exit unexpectedly during startup',
		async () => {
			...
			try {
				const { stdout: exitCode } = await execAsync(
					`podman ps -a --filter name=${CONTAINER_NAME} --format "{{.Status}}" | grep -o "Exited ([0-9]*)" | grep -o "[0-9]*" || echo ""`,
				);
