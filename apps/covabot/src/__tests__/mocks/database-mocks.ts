...
export class MockQdrantMemoryService {
	...
	async searchPersonalityNotes(
		...
		let results = this.personalityNotes.filter((note) => note.content.toLowerCase().includes(query.toLowerCase()));
	 ...
	 */
	async storeConversation(userId: string, channelId: string, content: string, metadata?: any): Promise<void> {
		if (this.shouldFail) {
			throw this.failureError;
		}
...
export interface MockMemoryService {
	...
	searchPersonalityNotes: jest.MockedFunction<
		(query: string, filters?: MemorySearchFilters, limit?: number) => Promise<PersonalityNote[]>
	>;
	updatePersonalityNote: jest.MockedFunction<
		(id: string, updates: Partial<PersonalityNote>) => Promise<PersonalityNote | null>
	>;
	deletePersonalityNote: jest.MockedFunction<(id: string) => Promise<boolean>>;
	storeConversation: jest.MockedFunction<
		(userId: string, channelId: string, content: string, metadata?: any) => Promise<void>
	>;
	generateEnhancedContext: jest.MockedFunction<
		(
			query: string,
			userId: string,
			channelId: string,
...
export function createMockMemoryService(): MockMemoryService {
	...
	return {
		...
		createPersonalityNote: jest
			.fn()
			.mockImplementation((content: string, metadata?: any) =>
				mockService.createPersonalityNote(content, metadata),
			),
		getPersonalityNote: jest.fn().mockImplementation((id: string) => mockService.getPersonalityNote(id)),
		searchPersonalityNotes: jest
			.fn()
			.mockImplementation((query: string, filters?: MemorySearchFilters, limit?: number) =>
				mockService.searchPersonalityNotes(query, filters, limit),
			),
		updatePersonalityNote: jest
			.fn()
			.mockImplementation((id: string, updates: Partial<PersonalityNote>) =>
				mockService.updatePersonalityNote(id, updates),
			),
		deletePersonalityNote: jest.fn().mockImplementation((id: string) => mockService.deletePersonalityNote(id)),
		storeConversation: jest
			.fn()
			.mockImplementation((userId: string, channelId: string, content: string, metadata?: any) =>
				mockService.storeConversation(userId, channelId, content, metadata),
			),
		generateEnhancedContext: jest
			.fn()
			.mockImplementation((query: string, userId: string, channelId: string, options?: any) =>
		...
		getNotes: jest.fn().mockImplementation((filters?: any) => {
			...
			return Promise.resolve(
				mockService.getMockData().personalityNotes.filter((note) => {
		...
		getNoteById: jest.fn().mockImplementation((id: string) => mockService.getPersonalityNote(id)),
		updateNote: jest
			.fn()
			.mockImplementation((id: string, request: any) => mockService.updatePersonalityNote(id, request)),
		deleteNote: jest.fn().mockImplementation((id: string) => mockService.deletePersonalityNote(id)),
		searchMemory: jest
			.fn()
			.mockImplementation((query: string, filters?: any) => mockService.searchPersonalityNotes(query, filters)),
...
export function createMockConfigurationService(): MockConfigurationService {
	...
	return {
		...
		setConfiguration: jest
			.fn()
			.mockImplementation((key: string, value: any) => mockService.setConfiguration(key, value)),
