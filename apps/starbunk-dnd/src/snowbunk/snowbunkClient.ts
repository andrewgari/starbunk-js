...
export default class SnowbunkClient extends DiscordClient {
	...
	public syncMessage = async (message: Message): Promise<void> => {
		...
		if (message.guild) {
			try {
				...
				if (blacklisted) {
					logger.debug(
						`Skipping message from blacklisted user ${message.author.id} in guild ${message.guild.id}`,
					);
					return;
				}
			} catch (error) {
		...
		try {
			...
			logger.error(
				'Error processing linked channels:',
				error instanceof Error ? error : new Error(String(error)),
			);
		}
	};
	...
	private writeMessage(message: Message, linkedChannel: TextChannel): void {
		...
		try {
			...
			try {
				...
				logger.warn(
					`[SnowbunkClient] Failed to use Discord service, falling back to direct message: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
			...
			logger.error(
				`[SnowbunkClient] Failed to send any message to channel ${linkedChannel.id}: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	public override async destroy(): Promise<void> {
		try {
			...
			logger.error(
				'Error during SnowbunkClient destroy:',
				error instanceof Error ? error : new Error(String(error)),
			);
		}
	}
