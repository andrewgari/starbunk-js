	...
	try {
		...
		if (!forceRefresh && identityCache.has(cacheKey)) {
			...
			if (age < CACHE_DURATION) {
				logger.debug(
					`[CovaIdentityService] Using cached identity for ${cacheKey} (age: ${Math.round(age / 1000)}s)`,
				);
		...
		logger.debug(
			`[CovaIdentityService] Fetching fresh identity for user ${COVA_USER_ID} in guild ${guildId || 'global'}`,
		);
		...
		if (!validateIdentity(identity)) {
			logger.error(
				`[CovaIdentityService] Identity validation failed for user ${COVA_USER_ID}: ${JSON.stringify(identity)}`,
			);
			return null;
		}
		...
		logger.debug(
			`[CovaIdentityService] Successfully cached identity: "${identity.botName}" with avatar ${identity.avatarUrl}`,
		);
		return identity;
	} catch (error) {
 ...
 */
async function fetchDiscordIdentity(userId: string, guildId?: string, message?: Message): Promise<BotIdentity | null> {
	try {
		...
		if (guildId && message) {
			...
			const botName = member.nickname || member.user.globalName || member.user.username || 'CovaBot';
 ...
 */
export async function preWarmIdentityCache(guildIds: string[]): Promise<void> {
