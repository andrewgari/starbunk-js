...
import {
	...
	createLLMEmulatorResponse,
	SimplePerformanceTimer,
} from '../simplifiedLlmTriggers';
...
// CI guard: skip real LLM API call tests in CI environments
const IS_CI = process.env.CI === 'true';
const itIfLocalRealLLM = IS_CI ? it.skip : it; // for tests that hit real LLM endpoints
const itIfNotCI = IS_CI ? it.skip : it; // generic CI skip for potentially networked tests
...
class EnhancedLLMResponseGenerator {
	...
	private async initializeLLMService(): Promise<void> {
		try {
			// In CI, do not attempt real LLM initialization (avoid network/API calls)
			if (IS_CI) {
				logger.debug('[EnhancedLLM] CI detected: skipping real LLM init, using fallback');
				return;
			}
...
describe('Enhanced LLM Triggers Tests', () => {
	...
	describe('LLM Service Integration', () => {
		...
		itIfLocalRealLLM(
			'should handle real LLM responses appropriately',
			async () => {
	...
	describe('Performance and Reliability', () => {
		...
		itIfLocalRealLLM('should handle concurrent requests efficiently', async () => {
			...
			const promises = Array(concurrentCount)
				.fill(0)
				.map(() => enhancedGenerator.generateResponse(testMessage));
		...
		it('should track performance metrics accurately', async () => {
			const testMessages = ['Quick test 1', 'Quick test 2', 'Quick test 3'];
