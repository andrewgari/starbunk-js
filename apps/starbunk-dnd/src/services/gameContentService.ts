...
export class GameContentService {
		...
		return Array.from(this.content.values())
			.filter((item) => {
				...
				if (options.searchText) {
					...
					if (!contentLower.includes(searchLower) && !tagsLower.some((t) => t.includes(searchLower))) {
						return false;
					}
				}
	...
	public async searchContent(searchText: string, campaign: Campaign, isGM: boolean): Promise<GameContent[]> {
