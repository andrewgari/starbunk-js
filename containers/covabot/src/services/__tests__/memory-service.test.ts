...
describe('Memory Service - Comprehensive Database Integration Tests', () => {
	...
	describe('Personality Note CRUD Operations', () => {
		...
		describe('Update Operations', () => {
			...
			it('should handle update failures gracefully', async () => {
				...
				await expect(memoryService.updatePersonalityNote('test-id', { content: 'test' }))
	...
	describe('Enhanced Context Generation', () => {
		...
		it('should filter conversation history by channel', async () => {
			...
			expect(context.conversationHistory.every((conv: any) =>
				conv.channelId === 'specific-channel' || conv.channelId === 'channel-123'
			)).toBe(true);
		});
		...
		it('should handle context generation failures gracefully', async () => {
			...
			await expect(memoryService.generateEnhancedContext('test', 'user', 'channel'))
	...
	describe('Conversation Storage', () => {
		it('should store conversation with user and channel context', async () => {
			...
			const storedConv = mockData.conversationHistory.find((conv: any) =>
				conv.userId === userId && conv.content === content
			);
		...
		it('should handle conversation storage failures gracefully', async () => {
			...
			await expect(memoryService.storeConversation('user', 'channel', 'content'))
	...
	describe('Data Persistence and Consistency', () => {
		...
		it('should handle concurrent operations safely', async () => {
			const promises = Array(5).fill(null).map((_, i) =>
				memoryService.createPersonalityNote(`Concurrent note ${i}`)
			);
	...
	describe('Performance and Scalability', () => {
		it('should handle large datasets efficiently', async () => {
			...
			const createPromises = Array(100).fill(null).map((_, i) =>
				memoryService.createPersonalityNote(`Performance test note ${i}`)
			);
	...
	describe('Error Recovery and Resilience', () => {
		...
		it('should handle partial operation failures', async () => {
			...
			await expect(memoryService.updatePersonalityNote(note.id, { content: 'Updated' }))
				.rejects.toThrow();
