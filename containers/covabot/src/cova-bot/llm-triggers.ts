...
// Minimal local prompts to satisfy linting and keep behavior predictable in this module
const COVA_BOT_PROMPTS = {
	EmulatorPrompt: "You are Cova. Emulate Cova's personality and respond concisely and helpfully.",
	DecisionPrompt: 'Decide if Cova should respond: reply only YES, LIKELY, UNLIKELY, or NO.',
} as const;
...
Message: ${message.content}`;
					...
					const response = await getLLMManager().createPromptCompletion(
						...
						userPrompt,
						{
							...
							contextData: {
								personalityEmbedding: personalityEmbedding
									? Array.from(personalityEmbedding)
									: undefined,
							},
						},
					);
					...
					if (response.length > 1900) {
						logger.warn(
							`[CovaBot] Response exceeded Discord's character limit (${response.length} chars), truncating`,
						);
...
Based on the Response Decision System, should Cova respond to this message?`;
					...
					if (
						...
						(messageContent.includes('cova') ||
							messageContent.includes('dev') ||
					...
					logger.error(
						`[CovaBot] Error in decision logic: ${error instanceof Error ? error.message : String(error)}`,
					);
