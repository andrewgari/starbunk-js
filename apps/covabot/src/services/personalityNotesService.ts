...
export class PersonalityNotesService {
	...
	async loadNotes(): Promise<void> {
		try {
			...
			logger.error(
				`[PersonalityNotes] Failed to load notes: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
			throw error;
		}
	}
	...
	private async saveNotes(): Promise<void> {
		try {
			...
			logger.error(
				`[PersonalityNotes] Failed to save notes: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
			throw error;
		}
	}
	...
	async getActiveNotesForLLM(): Promise<string> {
		...
		try {
			...
			logger.error(
				`[PersonalityNotes] Error getting configuration for LLM context: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
	...
	async getStats(): Promise<{
		...
		const byCategory = this.notes.reduce(
			(acc, note) => {
		...
		const byPriority = this.notes.reduce(
			(acc, note) => {
