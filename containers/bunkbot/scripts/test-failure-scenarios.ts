...
async function testDatabaseFailureScenarios() {
	...
	try {
		...
		const chadIdentity = await identityService.getBotIdentityByUsername('NonExistentChad', message as any, 'ChadBot');
	...
	try {
		...
		const chadIdentity = await failingIdentityService.getBotIdentityByUsername('Chad', message as any, 'ChadBot');
	...
	try {
		...
		const chadIdentity = await workingIdentityService.getChadIdentity(message as any);
