		...
		try {
			...
			if (!webhook) {
				try {
					...
					this.logger.warn(
						`Failed to create webhook "${webhookName}", falling back to regular message: ${webhookCreationError instanceof Error ? webhookCreationError.message : String(webhookCreationError)}`,
					);
			...
			this.logger.warn(
				`Failed to send webhook message: ${error instanceof Error ? error.message : String(error)}`,
			);
