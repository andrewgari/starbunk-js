...
class BluNiceRequestCondition extends PatternCondition {
	constructor() {
		super(Patterns.BLUE_NICE_REQUEST);
	}
	...
	const bot = new BotBuilder('BlueBot', webhookServiceToUse)
		...
		.withCustomCondition(
			...
			new AllConditions(
				new OneCondition(
					new PatternCondition(Patterns.BLUE),
					new PatternCondition(Patterns.BLUE_MEAN),
					new PatternCondition(Patterns.BLUE_ACKNOWLEDGMENT)
				),
		...
		.withConditionResponse(
			{
				generateResponse: async (message: Message) => {
					const name = niceRequestCondition.getNameFromMessage(message);
